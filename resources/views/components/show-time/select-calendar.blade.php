            ...
            <div class="ml-3 py-3">
                ...
                <div class="mt-3 ml-12 mr-8 grid grid-cols-3 gap-3 text-center"  >
                    @foreach ($dates as $calendar)
                        <a href="{{route('book_ticket',['movie_slug'=>$movie->slug,'calendar'=>$calendar])}}"class="border border-white p-1 cursor-pointer">{{ date('H:i', strtotime($calendar->time_start))}}</a>
