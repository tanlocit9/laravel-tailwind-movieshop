...
class BookTicket extends Component
{
    ...
    public function mount($movie, $calendar)
    {
        ...
        $this->amount = $this->user->getBillAmountByCalendar($this->calendar->id);
    ...
    public function decrease($price_id)
    {
        ...
        if (Price::checkIsTicketType($price_id)) {
            $this->sessionTickets--;
            session()->forget('totalTicket');
            session()->put('totalTicket', $this->sessionTickets);
        }
    }
    ...
    public function countPrice($prices)
    {
        ...
        foreach ($prices as $price) {
            if ($this->amount[$price->id] == "" || !str_contains($this->amount[$price->id], "-")) {
                $this->total_price[$price->id] = (int) $this->amount[$price->id] * $price->price;
            } else {
                $this->total_price[$price->id] = 0;
                $this->amount[$price->id] = 0;
            }
        }
        for ($i = 1; $i <= 3; $i++) {
        ...
        session()->put('sessiontotalPriceCombos', $this->total_ticket);
    }
